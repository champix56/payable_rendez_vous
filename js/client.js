...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const textareaQuestions = document.querySelector('#client_take_apointment_question_content');

    if (undefined != textareaQuestions) {
        //console.log(select);
        // debugger;

        textareaQuestions.addEventListener('change', getQuestionsValidity);
    }
});
function getQuestionsValidity(evt){
...
function getConsultantDatas(evt) {
    ...
    if (evt.target.value === '-1') {
        ...
        if (questionIsOpen) jQuery('.questions_content').slideUp(400,() => { questionIsOpen = false })
        return;
    }
...
function getPrestationTypeDatas(evt) {
    ...
    wpAdminFetch("/wp-json/rdv_plugin/v1/prestations/" + evt.target.value)
        ...
        .then(r => {
            ...
            if (!questionIsOpen) jQuery('.questions_content').slideDown( 400,() => { questionIsOpen = true })
        });
}
